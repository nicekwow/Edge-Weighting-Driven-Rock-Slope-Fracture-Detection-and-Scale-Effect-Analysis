...
@TRANSFORMS.register_module()
class JointAwareRandomCrop(RandomCrop):
    """Crop around foreground pixels with a configurable probability.

    Most patches can be forced to contain a target class while a small
    proportion of uniformly random patches is retained for background
    learning.

    Args:
        crop_size (Union[int, Tuple[int, int]]): Crop size in ``(h, w)``.
        foreground_label (int): Label used as foreground. Defaults to 1.
        foreground_prob (float): Probability of foreground-aware sampling.
            The remaining probability uses uniform random cropping.
            Defaults to 0.8.
        min_foreground_pixels (int): Preferred minimum number of foreground
            pixels. If it cannot be reached after ``max_attempts``, the
            candidate containing the most foreground is used. Defaults to 512.
        max_attempts (int): Maximum foreground-aware sampling attempts.
            Defaults to 20.
    ...
    def _uniform_bbox(self, img: np.ndarray) -> tuple:
        """Generate a uniformly random crop box."""
        margin_h = max(img.shape[0] - self.crop_size[0], 0)
        margin_w = max(img.shape[1] - self.crop_size[1], 0)
        offset_h = np.random.randint(0, margin_h + 1)
        offset_w = np.random.randint(0, margin_w + 1)
        return (offset_h, offset_h + self.crop_size[0], offset_w,
                offset_w + self.crop_size[1])

    def _foreground_bbox(self, img: np.ndarray, y: int, x: int) -> tuple:
        """Generate a random crop box containing a foreground point."""
        margin_h = max(img.shape[0] - self.crop_size[0], 0)
        margin_w = max(img.shape[1] - self.crop_size[1], 0)

        min_y = max(0, y - self.crop_size[0] + 1)
        max_y = min(y, margin_h)
        min_x = max(0, x - self.crop_size[1] + 1)
        max_x = min(x, margin_w)
        offset_h = 0 if min_y > max_y else np.random.randint(
            min_y, max_y + 1)
        offset_w = 0 if min_x > max_x else np.random.randint(
            min_x, max_x + 1)
        return (offset_h, offset_h + self.crop_size[0], offset_w,
                offset_w + self.crop_size[1])
    ...
    @cache_randomness
    def crop_bbox(self, results: dict) -> tuple:
        """Choose a uniform crop or a crop around the target foreground."""
        img = results['img']
        seg_map = results['gt_seg_map']
        foreground = np.argwhere(seg_map == self.foreground_label)

        if (foreground.size == 0
                or np.random.rand() >= self.foreground_prob):
            return self._uniform_bbox(img)

        best_bbox = None
        best_count = -1
        for _ in range(self.max_attempts):
            y, x = foreground[np.random.randint(0, len(foreground))]
            crop_bbox = self._foreground_bbox(img, int(y), int(x))
            seg_crop = self.crop(seg_map, crop_bbox)
            foreground_count = int(
                np.count_nonzero(seg_crop == self.foreground_label))
